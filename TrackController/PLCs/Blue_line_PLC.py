...
def main(stop_event, block_occupancies, switch_suggestions, switches, traffic_lights, crossings, speed_hazard): # block_occupancy_bools, switch_position_bools, speed_bool, plc_outputs, plc_output_lock
    """Main loop of the PLC program that processes inputs and updates outputs."""
    while not stop_event.is_set():
        
        # logic to control crossing
        if block_occupancies[1] or block_occupancies[2] or block_occupancies[3]:
        ...
        if block_occupancies[5]:
            if switch_suggestions[0] == False:
                if not(block_occupancies[7] or block_occupancies[8] or block_occupancies[9] or block_occupancies[10]):
                    switches[0] = False
                    traffic_lights[0] = True
                    traffic_lights[1] = False

                elif not(block_occupancies[12] or block_occupancies[13] or block_occupancies[14] or block_occupancies[15]):
                ...
                if not(block_occupancies[12] or block_occupancies[13] or block_occupancies[14] or block_occupancies[15]):
                    switches[0] = True
                    traffic_lights[0] = False
                    traffic_lights[1] = True

                elif not(block_occupancies[7] or block_occupancies[8] or block_occupancies[9] or block_occupancies[10]):
